...
const char* cmaes::SayHello(void) {
  /* write initial message */
	char buf[333];
  sprintf(buf, 
          "(%d,%d)-CMA-ES(mu_eff=%.1f), Ver=\"%s\", dimension=%d, diagonalIterations=%ld, randomSeed=%d @ %s", 
          rp.mu, rp.lambda, rp.mueff, version.c_str(), rp.N, (long)rp.diagonalCov,
          rp.seed, getTimeStr());
...
cmaes::cmaes(int dimension, double *inxstart, double *instddev, long inseed, int lambda,
			 const char *input_parameter_filename)
{
	t = new cmaes_t();
	version = "3.11.00.beta (C++)";
	readpara_init (&rp, dimension, inseed, inxstart, instddev, 
                   lambda, input_parameter_filename);
	
	rp.seed = random_init( &t->rand, (long unsigned int) rp.seed);
	int N = rp.N; /* for convenience */
  ...
  for (i = 0, trace = 0.; i < N; ++i)
    trace += rp.rgInitialStds[i]*rp.rgInitialStds[i];
  t->sigma = sqrt(trace/N); /* rp.mueff/(0.2*rp.mueff+sqrt(N)) * sqrt(trace/N); */
  ...
  t->publicFitness = new_double(rp.lambda); 
  t->rgFuncValue = new_double(rp.lambda+1); 
  t->rgFuncValue[0]=rp.lambda; ++t->rgFuncValue;
  t->arFuncValueHist = new_double(10+(int)ceil(3.*10.*N/rp.lambda)+1);
  t->arFuncValueHist[0] = (double)(10+(int)ceil(3.*10.*N/rp.lambda));
  t->arFuncValueHist++; 
  ...
  t->index = (int *) new_void(rp.lambda, sizeof(int));
  for (i = 0; i < rp.lambda; ++i) 
    t->index[i] = i; /* should not be necessary */
  t->rgrgx = (double **)new_void(rp.lambda, sizeof(double*));
  for (i = 0; i < rp.lambda; ++i) {
  ...
  for (i = 0; i < N; ++i)
    {
      t->B[i][i] = 1.;
      t->C[i][i] = t->rgD[i] = rp.rgInitialStds[i] * sqrt(N / trace);
      t->C[i][i] *= t->C[i][i];
      t->rgpc[i] = t->rgps[i] = 0.;
    }
  ...
  /* set xmean */
  for (i = 0; i < N; ++i)
    t->rgxmean[i] = t->rgxold[i] = rp.xstart[i]; 
  /* use in case xstart as typicalX */
  if (rp.typicalXcase) 
    for (i = 0; i < N; ++i)
      t->rgxmean[i] += t->sigma * t->rgD[i] * random_Gauss(&t->rand);

  if (strcmp(rp.resumefile, "_no_")  != 0)
	  resume_distribution(rp.resumefile);
...
void 
cmaes::resume_distribution(char *filename)
{
  int i, j, res, n; 
  double d; 
...
/* --------------------------------------------------------- */
double const * 
cmaes::SetMean(const double *xmean)
/*
...
/* --------------------------------------------------------- */
double * const * 
cmaes::SamplePopulation(void)
{
  int iNk, i, j, N=rp.N;
  int flgdiag = ((rp.diagonalCov == 1) || (rp.diagonalCov >= t->gen)); 
  double sum;
...
/* --------------------------------------------------------- */
double const * 
cmaes::ReSampleSingle_old( double *rgx)
{
  int i, j, N=rp.N;
  double sum; 
...
/* --------------------------------------------------------- */
double * const * 
cmaes::ReSampleSingle( int iindex)
{
  int i, j, N=rp.N;
  double *rgx; 
  double sum; 
  ...
  if (iindex < 0 || iindex >= rp.lambda) {
    sprintf(s, "index==%d must be between 0 and %d", iindex, rp.lambda);
...
/* --------------------------------------------------------- */
double * 
cmaes::SampleSingleInto( double *rgx)
{
  int i, j, N=rp.N;
  double sum; 
...
/* --------------------------------------------------------- */
double * 
cmaes::PerturbSolutionInto( double *rgx, double const *xmean, double eps)
{
  int i, j, N=rp.N;
  double sum; 
...
/* --------------------------------------------------------- */
double *
cmaes::UpdateDistribution(const double *rgFunVal)
{
  int i, j, iNk, hsig, N=rp.N;
  int flgdiag = ((rp.diagonalCov == 1) || (rp.diagonalCov >= t->gen)); 
  double sum; 
  double psxps; 
  ...
  /* Generate index */
  Sorted_index(rgFunVal, t->index, rp.lambda);
  
  /* Test if function values are identical, escape flat fitness */
  if (t->rgFuncValue[t->index[0]] == 
      t->rgFuncValue[t->index[(int)rp.lambda/2]]) {
    t->sigma *= exp(0.2+rp.cs/rp.damps);
  ...
  for (i = 0; i < N; ++i) {
    ...
    t->rgxmean[i] = 0.;
    for (iNk = 0; iNk < rp.mu; ++iNk) 
      t->rgxmean[i] += rp.weights[iNk] * t->rgrgx[t->index[iNk]][i];
    t->rgBDz[i] = sqrt(rp.mueff)*(t->rgxmean[i] - t->rgxold[i])/t->sigma; 
  }
  ...
  */
  ...
  for (i = 0; i < N; ++i) {
    if (!flgdiag)
      ...
      sum = t->rgdTmp[i];
    t->rgps[i] = (1. - rp.cs) * t->rgps[i] + 
      sqrt(rp.cs * (2. - rp.cs)) * sum;
  }
  ...
  hsig = sqrt(psxps) / sqrt(1. - pow(1.-rp.cs, 2*t->gen)) / t->chiN
    < 1.4 + 2./(N+1);
  for (i = 0; i < N; ++i) {
    t->rgpc[i] = (1. - rp.ccumcov) * t->rgpc[i] + 
      hsig * sqrt(rp.ccumcov * (2. - rp.ccumcov)) * t->rgBDz[i];
  }
  
  /* stop initial phase */
  if (t->flgIniphase && 
      t->gen > douMin(1/rp.cs, 1+N/rp.mucov)) 
    {
      if (psxps / rp.damps / (1.-pow((1. - rp.cs), t->gen)) 
          < N * 1.05) 
        t->flgIniphase = 0;
    }
  ...
  if (rp.ccov != 0. && t->flgIniphase == 0) {
    ...
    for (i = 0; i < N; ++i)
      for (j = 0; j <=i; ++j) {
        t->C[i][j] = (1 - rp.ccov) * t->C[i][j] 
          + rp.ccov * (1./rp.mucov) 
            * (t->rgpc[i] * t->rgpc[j] 
               + (1-hsig)*rp.ccumcov*(2.-rp.ccumcov) * t->C[i][j]);
        for (k = 0; k < rp.mu; ++k) /* additional rank mu update */
          t->C[i][j] += rp.ccov * (1-1./rp.mucov) * rp.weights[k]  
  ...
  /* update of sigma */
  t->sigma *= exp(((sqrt(psxps)/t->chiN)-1.)*rp.cs/rp.damps);
...
void cmaes::Adapt_C2(int hsig)
{
  ...
  if (rp.ccov != 0. && t->flgIniphase == 0) {
    ...
    double ccov1 = douMin(rp.ccov * (1./rp.mucov) * (flgdiag ? (N+1.5) / 3. : 1.), 1.);
    double ccovmu = douMin(rp.ccov * (1-1./rp.mucov)* (flgdiag ? (N+1.5) / 3. : 1.), 1.-ccov1); 
...
{
  int i, N = rp.N; 
  if (rp.rgDiffMinChange == NULL)
    return;

  for (i = 0; i < N; ++i)
    while (t->sigma * sqrt(t->C[i][i]) < rp.rgDiffMinChange[i]) 
      t->sigma *= exp(0.05+rp.cs/rp.damps);
...
/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
void cmaes::WriteToFile(const char *key, const char *name)
{ 
  WriteToFileAW(key, name, "a"); /* default is append */
}

/* --------------------------------------------------------- */
/* --------------------------------------------------------- */
void cmaes::WriteToFileAW(const char *key, const char *name, 
                         char *appendwrite)
{ 
  char *s = "tmpcmaes.dat"; 
  FILE *fp;
  ...
  if (appendwrite[0] == 'w') {
    ...
    fprintf(fp, "%% # %s (randomSeed=%d, %s)\n", key, rp.seed, getTimeStr());
  } else 
    if (t->gen > 0 || strncmp(name, "outcmaesfit", 11) != 0)
      WriteToFilePtr(key, fp); /* do not write fitness for gen==0 */
...
/* --------------------------------------------------------- */
void cmaes::WriteToFilePtr(const char *key, FILE *fp)
  ...
  while (key < keyend)
    {
      ...
      if (strncmp(key, "fmedian", 7) == 0)
        {
          fprintf(fp, "%.15e", t->rgFuncValue[t->index[(int)(rp.lambda/2)]]);
          while (*key != '+' && *key != '\0' && key < keyend)
            ++key;
          fprintf(fp, "%c", (*key=='+') ? '\t':'\n');
        }
      if (strncmp(key, "fworst", 6) == 0)
        {
          fprintf(fp, "%.15e", t->rgFuncValue[t->index[rp.lambda-1]]);
          while (*key != '+' && *key != '\0' && key < keyend)
            ++key;
          fprintf(fp, "%c", (*key=='+') ? '\t':'\n');
        }
      ...
      s0 = key;
      d = Get(key); /* TODO find way to detect whether key was found */
...
/* --------------------------------------------------------- */
double * 
cmaes::GetInto( char const *s, double *res)
{
  int i, N = rp.N;
  double const * res0 = GetPtr(s);
  if (res == NULL)
    res = new_double(N);
  for (i = 0; i < N; ++i)
    res[i] = res0[i];
...
/* --------------------------------------------------------- */
double * 
cmaes::GetNew( char const *s)
{
  return GetInto( s, NULL);
}

/* --------------------------------------------------------- */
const double * 
cmaes::GetPtr( char const *s)
{
  int i, N=rp.N;
...
/* --------------------------------------------------------- */
double * 
cmaes::GetPublicFitness(void) {
	return (t->publicFitness); 
}

/* --------------------------------------------------------- */
int
cmaes::GetDimension(void) {
	return rp.N;
}
...
cmaes::TestForTermination(void)
{
  ...
  cp[0] = '\0';
      ...
      if ((t->gen > 1 || t->state > 1) && rp.stStopFitness.flg && 
          t->rgFuncValue[t->index[0]] <= rp.stStopFitness.val) 
      ...
      range = douMax(rgdouMax(t->arFuncValueHist, (int)douMin(t->gen,*(t->arFuncValueHist-1))), 
                     rgdouMax(t->rgFuncValue, rp.lambda)) -
        douMin(rgdouMin(t->arFuncValueHist, (int)douMin(t->gen, *(t->arFuncValueHist-1))), 
               rgdouMin(t->rgFuncValue, rp.lambda));
      
      if (t->gen > 0 && range <= rp.stopTolFun) {
        cp += sprintf(cp, 
      ...
      if (t->gen > *(t->arFuncValueHist-1)) {
        ...
        if (range <= rp.stopTolFunHist)
          cp += sprintf(cp, 
                        "TolFunHist: history of function value changes %7.2e stopTolFunHist=%7.2e", 
                        range, rp.stopTolFunHist);
      }
      
      /* TolX */
      for(i=0, cTemp=0; i<N; ++i) {
        cTemp += (t->sigma * sqrt(t->C[i][i]) < rp.stopTolX) ? 1 : 0;
        cTemp += (t->sigma * t->rgpc[i] < rp.stopTolX) ? 1 : 0;
      }
      if (cTemp == 2*N) {
        cp += sprintf(cp, 
      ...
      /* TolUpX */
      for(i=0; i<N; ++i) {
        if (t->sigma * sqrt(t->C[i][i]) > rp.stopTolUpXFactor * rp.rgInitialStds[i])
          break;
      }
      if (i < N) {
        cp += sprintf(cp, 
      ...
      /* if (flg) t->sigma *= exp(0.05+rp.cs/rp.damps); */

      if(t->countevals >= rp.stopMaxFunEvals) 
        cp += sprintf(cp, "MaxFunEvals: conducted function evaluations %.0f >= %g\n", 
                      t->countevals, rp.stopMaxFunEvals);
      if(t->gen >= rp.stopMaxIter) 
        cp += sprintf(cp, "MaxIter: number of iterations %.0f >= %g\n", 
                      t->gen, rp.stopMaxIter); 
      if(t->flgStop)
...
/* --------------------------------------------------------- */
void cmaes::ReadSignals(char const *filename)
{
...
/* --------------------------------------------------------- */
void cmaes::ReadFromFilePtr( FILE *fp)
          ...
          if((nb=sscanf(s, keys[ikey], sin1, sin2, sin3, sin4)) >= 1) 
            {
              ...
              case 0 : /* "stop", reads "stop now" or eg. stopMaxIter */
                if (strncmp(sin1, "now", 3) == 0) 
                  ...
                  if (sscanf(sin2, " %lg", &d) == 1) 
                    {
                      rp.stStopFitness.flg = 1; 
                      rp.stStopFitness.val = d; 
                    }
                }
              ...
              case 2 : /* "write" */
                ...
                if (t->countevals < rp.lambda && t->flgresumedone == 0) 
                  WriteToFileAW(sin1, sin2, "w"); /* overwrite */
...
/* --------------------------------------------------------- */
void 
cmaes::UpdateEigensystem(int flgforce)
{
  int i, N = rp.N;
  ...
  if(flgforce == 0) {
    ...
    if (rp.updateCmode.flgalways == 0 /* not implemented, always ==0 */
        && t->gen < t->genOfEigensysUpdate + rp.updateCmode.modulo
        )
      return;

    /* return on time percentage */
    if (rp.updateCmode.maxtime < 1.00 
        && t->eigenTimings.tictoctime > rp.updateCmode.maxtime * t->eigenTimings.totaltime
...
/// Clone method.
base_ptr cma_es::clone() const
{
	return base_ptr(new cma_es(*this));
}
...
void cma_es::evolve(population &pop) const
{
	...
	// Create object if not existing yet
	if (! cma) {
		cma = new cmaes(D, NULL, NULL, 0, 0, "initials.par");;
	}
	// else check if size is right ...
...
/*	// Initialise the chromosomes and their fitness to that of the initial deme
	...
	// should we check if the input is correct?! 
	// incoming pop = the one from last time?
	
	cma->ReadSignals("signals.par");

	double *const*Xnew = cma->SamplePopulation();

	decision_vector v(D);
	
	for (int i = 0; i <	NP; i++) {
		for(int j = 0; j < D; j++) v[j] = Xnew[i][j];
		
		while(! prob.verify_x(v)) {
			Xnew = cma->ReSampleSingle(i);
			for(int j = 0; j < D; j++) v[j] = Xnew[i][j];			
		}
...
/// Algorithm name
std::string cma_es::get_name() const
{
	return "A Simple CMA ES Implementation/Wrapper";
}
 ...
 */
std::string cma_es::human_readable_extra() const
{
	std::ostringstream s;
/*	s << "gen:" << m_gen << ' ';
	s << "CR:" << m_cr << ' ';
...
BOOST_CLASS_EXPORT_IMPLEMENT(pagmo::algorithm::cma_es);

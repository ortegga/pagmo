...
namespace pagmo
{
  namespace odeint
  {
    ...
    {
    public:
      integrator (cuda::info & inf, size_t individuals, size_t task_count_) : cuda::task<ty>(inf, individuals, task_count_, 1), 
	...
	if (inputs.size() == get_size())
	  {
	    return task<ty>::set_individual_inputs (individual, taskid, this->param_x, inputs);
	  }
	return false;
      }
	...
	if (inputs.size() == 2)//<TODO> incomplete type
	  {
	    return cuda::task<ty>::set_individual_inputs (individual, taskid, this->param_o, inputs);
	  }
	return false;
      }
      ...
      {
	return task<ty>::get_individual_outputs (individual, taskid, this->param_x, outputs);
      }

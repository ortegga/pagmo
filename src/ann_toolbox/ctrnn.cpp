...
ctrnn::ctrnn(unsigned int input_nodes_, unsigned int hidden_nodes_, unsigned int output_nodes_,
	const std::vector<CUDA_TY> &w) : 
    	neural_network(input_nodes_, output_nodes_),
		m_hidden(hidden_nodes_),
		m_time_step(0.1),
		m_hidden_neurons(std::vector<CUDA_TY>(hidden_nodes_, 0)),
		m_output_neurons(std::vector<CUDA_TY>(output_nodes_, 0))
{
...
void ctrnn::set_weights(const std::vector<CUDA_TY> &w) {
    ...
    for (int i = 0; i < m_weights.size(); i++) 
		m_weights[i] = (m_weights[i] + (uppbound_weights-lowbound_weights)*.5) / 
		  (uppbound_weights-lowbound_weights);
...
const std::vector<CUDA_TY> ctrnn::compute_outputs(std::vector<CUDA_TY> &inputs) {
	...
	std::vector<CUDA_TY> tempH(m_hidden, 0);
	for(i = 0; i < m_hidden; i++) {

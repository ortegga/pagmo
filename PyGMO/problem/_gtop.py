from _problem import cassini_1, gtoc_1,cassini_2, rosetta, messenger_full, tandem, laplace, sagas, mga_1dsm_alpha, mga_1dsm_tof, mga_incipit, mga_incipit_cstrs, mga_incipit_cstrs_wrapped, mga_part
...
def _mga_1dsm_alpha_ctor(self, seq = [planet_ss('earth'),planet_ss('venus'),planet_ss('earth')], t0 = [epoch(0),epoch(1000)], tof = [365.25,5.0 * 365.25], vinf = [0.5, 2.5], multi_objective = False, add_vinf_dep = False, add_vinf_arr = True):
	"""
...
def _mga_incipit_ctor(self, seq = [planet_js('io'),planet_js('io'),planet_js('europa')], t0 = [epoch(7305.0),epoch(11323.0)],tof = [[100,200],[3,200],[4,100]]):
	"""
...
def _mga_incipit_cstrs_ctor(self, seq = [planet_js('io'),planet_js('io'),planet_js('europa')], t0 = [epoch(7305.0),epoch(11323.0)],tof = [[100,200],[3,200],[4,100]], Tmax = 365.25, Dmin = 2.0):
	"""
...
def _mga_incipit_cstrs_wrapped_ctor(self, seq = [planet_js('io'),planet_js('io'),planet_js('europa')], t0 = [epoch(7305.0),epoch(11323.0)],tof = [[100,200],[3,200],[4,100]], Tmax = 365.25, Dmin = 2.0):
	"""
	USAGE: mga_incipit_cstrs_wrapped(seq = [planet_js('io'),planet_js('io'),planet_js('europa')], t0 = [epoch(6905.0),epoch(11323.0)], tof = [[100,200],[3,200],[4,100]], Tmax = 365.25, Dmin = 0.2)
	...
	* tof: list of n lists containing the lower and upper bounds for the legs flight times (days)
	...
	arg_list.append(Dmin)
	self._orig_init(*arg_list)
mga_incipit_cstrs_wrapped._orig_init = mga_incipit_cstrs_wrapped.__init__
mga_incipit_cstrs_wrapped.__init__ = _mga_incipit_cstrs_wrapped_ctor
...
def _mga_part_plot(self,x):
	...
	for i in xrange(0,legs):
		...
		plot_kepler(ax,r_P[i],v_out,x[4*i+2]*T[i]*DAY2SEC,common_mu,N = 500, color='b', legend=False, units = JR)
		#Lambert arc to reach Earth during (1-nu2)*T2 (second segment)
		dt = (1-x[4*i+2])*T[i]*DAY2SEC
